...
| 9 | Instructions in the file, reasons outside it | A defended choice describes the draft, not the current state, and an alternative the reader never considered stays in context as text. The consequence that rule 3 requires is the reason that stays | "this is intentional", "the reason … is", a clause that defends a choice the document records, a negation of an alternative nothing points the reader at, an absence explained in the document rather than in the world |
