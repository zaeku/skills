...
commit is not the working copy. `jj status` then reports **no conflict**, and it still
lists your own working-copy changes. A bare `jj resolve --list` exits with
...
`jj resolve --list` does succeed. Neither output proves the conflict is gone.
